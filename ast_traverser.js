...
const createCipherSession = () => {
    ...
    const alphabet = shuffle(pool).slice(0, 16);
...
const encodeWithSession = (session, value) => {
    ...
    for (let i = 0; i < str.length; i++) {
        const byte = (str.charCodeAt(i) + session.key + (i % 17)) % 256;
...
const transformAst = async (code) => {
    ...
    let output = minifyLuau(astToCode(ast));
    if (state.hasEncryptedStrings) {
        output = minifyLuau(`${createDecoderRuntime(state.cipher)};${output}`);
    }
    return {
        code: output,
        hasEncryptedStrings: false
